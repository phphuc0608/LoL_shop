    ...
    <div class="sort_cotainer p-3 ">
      <form action="{{url('tim_kiem_item_process')}}" method="post">
        ...
        <div class="form-group" style="margin-bottom: 90px!important">
            ...
            <input class="form-control mb-2" type="search" name="ten_vat_pham" id="ten_vat_pham">
        ...
        <table class="table table-striped">
            ...
            <tbody>
              @foreach ($items as $item)
                <tr>
                    <th class="text-center">{{$item->ma_vat_pham}}</th>
                    <th class="text-center">{{$item->ten_vat_pham}}</th>
                    <th class="text-center">{{$item->loai_vat_pham->ten_loai_vat_pham}}</th>
                    <th class="text-center">{{$item->loai_vat_pham->gia}}</th>
                    <th class="text-center"><?php echo $item->trang_thai == 1?'Đang bán':'Ngừng bán' ?></th>
                    <th class="text-center"><img style="width: 80px; height: 150px;" src="{{asset('item/'.$item->hinh_anh)}}" alt=""></th>
                    <th class="text-center">
                        <button class="table_btn" data-toggle="modal" data-target="#update_item"><i class="bi bi-pencil update_icon"></i></button>
                        |
                        <button class="table_btn"><a href="{{route('xoa_item',['ma_vat_pham'=>$item->ma_vat_pham])}}"><i class="bi bi-trash3 remove_icon"></i></a></button>
                    </th>
                </tr>
              @endforeach
            </tbody>
    ...
    <div class="col-md-12">
      <div class="pagination d-flex justify-content-center">
        <ul class="pagination">
          <li class="page-item">
            @if($page > 1)
                <a class="previous page-link" href="{{route('quan_ly_item',['page'=>($page-1)])}}">&lt;</a>
            @endif
          </li>
            @for($i = 1; $i <= $page_number; ++$i)
              <li class="page-item">
                <a class="page-link" href="{{route('quan_ly_item',['page'=>$i])}}">{{$i}}</a>  
              </li>
            @endfor
          <li class="page-item">
            @if($page < $page_number)
              <a class="next page-link" href="{{route('quan_ly_item',['page'=>($page+1)])}}">&gt;</a>
            @endif
          </li>
        </ul>
      </div>
      ...
      <div class="modal-body">
        <form action="{{url('them_item')}}" method="post" enctype="multipart/form-data">
          ...
          <div class="form-group">
            ...
            <input name="ten_item" type="text" class="form-control" id="ten_item" placeholder="Nhập tên vật phẩm">
          </div>
          <div class="form-group">
      ...
      <div class="modal-body">
        <form action="{{url('sua_item')}}" method="post" enctype="multipart/form-data">
          ...
          <div class="form-group">
            ...
            <input name="ma_item" type="text" class="form-control" id="ma_item" readonly value="{{$item->ma_vat_pham}}">
          </div>
          <div class="form-group">
            <label for="up_ten_item">Tên vật phẩm</label>
            <input name="up_ten_item" type="text" class="form-control" id="up_ten_item" value="{{$item->ten_vat_pham}}">
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <select class="form-control" id="up_loai_vat_pham" name="up_loai_vat_pham">
              @foreach($loai_vat_phams as $loai_vat_pham)
                <option value="{{$loai_vat_pham->ma_loai_vat_pham}}" {{$item->ma_loai_vat_pham==$loai_vat_pham->ma_loai_vat_pham?" selected":""}}>{{$loai_vat_pham->ten_loai_vat_pham}}</option>
              @endforeach
            </select>
          </div>
          <div class="form-group">
            ...
            <select class="form-control" id="up_state" name="up_state">
              <option value="1" {{$item->trang_thai=="1"?" selected":""}}>Đang bán</option>
              <option value="0" {{$item->trang_thai=="0"?" selected":""}}>Ngừng bán</option>
            </select>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-dismiss="modal">Đóng</button>
